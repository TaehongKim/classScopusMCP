...
# -*- coding: utf-8 -*-
"""
Scopus API + Crossref API를 활용한 논문 검색 (SSL 문제 해결)
"""
...
class ScopusWithCrossrefClient:
    """Scopus API + Crossref API 클라이언트 (SSL 문제 해결)"""
    ...
    def get_abstract_from_crossref(self, doi: str) -> str:
        """Crossref API를 사용하여 DOI로부터 초록 가져오기 (SSL 검증 비활성화)"""
    ...
    def get_paper_info_from_crossref(self, doi: str) -> Dict:
        """Crossref API를 사용하여 논문 정보 가져오기 (SSL 검증 비활성화)"""
...
def test_crossref_doi_ssl_fixed():
    """Crossref API 테스트 (SSL 문제 해결)"""
    ...
    print("🔍 Crossref API 테스트 (SSL 문제 해결)...")
    print("=" * 60)
...
def search_usability_papers_with_crossref_ssl_fixed():
    """Usability 관련 논문 검색 (Crossref 초록 포함, SSL 문제 해결)"""
    
    api_key = "920a284740d2c60fc3249e6e795e928c"
    client = ScopusWithCrossrefClient(api_key)
    
    print("🔍 Usability 논문 검색 시작 (Crossref 초록 포함, SSL 문제 해결)...")
    print("=" * 60)
...
def save_to_csv(papers: List[Dict], filename: str = "scopus_crossref_ssl_fixed.csv"):
    """결과를 CSV 파일로 저장"""
...
def main():
    ...
    print("🔍 Scopus + Crossref 논문 검색 시스템 (SSL 문제 해결)")
    print("=" * 60)
    
    try:
        # 먼저 Crossref API 테스트
        print("1. Crossref API 테스트 (SSL 문제 해결)...")
        test_crossref_doi_ssl_fixed()
        
        print("\n" + "=" * 60)
        print("2. Usability 논문 검색 (Crossref 초록 포함)...")
        
        # Usability 논문 검색 (Crossref 초록 포함)
        papers = search_usability_papers_with_crossref_ssl_fixed()
        
        if papers:
            # CSV 저장
            save_choice = input("\n결과를 CSV 파일로 저장하시겠습니까? (y/n): ").strip().lower()
            if save_choice == 'y':
                filename = input("파일명을 입력하세요 (기본: scopus_crossref_ssl_fixed.csv): ").strip()
                if not filename:
                    filename = "scopus_crossref_ssl_fixed.csv"
